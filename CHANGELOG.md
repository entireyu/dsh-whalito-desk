...
### 新增
- 适配 DSH web 0.1.2+：token/会话认证握手、网关协议（桌宠轮询改 `session/list` + `{args:{_request:{}}}`），会话 cookie 以 SameSite=None 注入 WebView2——内嵌页面不再 401 白屏。
- 管理后台可检查/更新鲸仔（不再依赖 DSH 内嵌页）；服务器启动失败（含连续自动重启失败）给出「尝试更新鲸仔」「管理插件」恢复动作。
- 「重新运行安装引导」改为环境体检向导：只检测/安装/修复，不再自动启动服务器或跳内嵌页。
- 插件管理（应急保启动）：自动枚举 bundle 层已安装插件（含插件市场安装的），可禁用/启用；禁用写入 `cordis.patch.yml` 的 disabled 覆盖行、重启生效；内置入口插件单独说明「不提供禁用」。
- 运行日志结构化：类型徽标/时间/摘要，行尾「展开/收起」，错误行与批量「复制」「复制到 AI」（带 DSH/鲸仔版本提示词），分页（默认尾 300 条 + 加载更早），自动滚动仅在贴底时生效。
- 会话 cookie 持久化：退出后服务器若继续运行，重进时无缝接回（服务器不重启）；无 cookie 的旧残留自动清理后重启。
- 测试构建右键菜单加「开发者工具」；管理后台面板右键直接打开开发者工具（排障用）。
- 服务信息卡片状态与 WebUI 地址分行展示，状态不再被超长 token 地址挤成省略号。

### 变更
- 管理后台布局两列 3:2：左列=三状态卡 + 操作按钮 + 运行日志，右列=插件管理 / 插件市场 / 关于。
- 服务器操作按钮重排：停止/重启并排；主按钮改为「进入 DeepSeek Harness」并加大加高；在浏览器打开 / 复制地址收进「⋯」下拉。
- 插件状态文案「加载中」改为「已启用 / 已禁用（重启后不加载）」。
- 「关于」卡片去掉「隐藏到托盘」按钮（关闭窗口即隐藏到托盘）。

### 修复
- 主窗口内嵌 DSH 401 白屏（跨站 iframe 拿不到 SameSite=Strict 会话 cookie）。
- 退出鲸仔后重进白屏（服务器残留、拿不到旧进程 token）；遗留托管服务器自愈清理重启。
- 桌宠轮询 `session.list` 404（方法改名）与原生客户端认证（握手 303）。
- macOS CI 编译失败（`Manager` trait 全平台导入）。
- 左列状态卡与操作按钮重叠（`.server-tools` flex-wrap 冲突）。
- 运行日志整行点击展开与文字选中冲突（改为仅行尾按钮）。
- 服务信息卡片「状态」被超长地址挤压成省略号。

## [0.5.1] - 2026-08-30

### 新增
- 「鲸仔设置」→「DeepSeek Harness 服务设置」新增 **服务跟随鲸仔程序停止**（默认关）：开启后，用户退出鲸仔（托盘 / 桌宠「退出」）时同时停止由鲸仔启动的 DSH 服务（退出前强制停服并短等端口释放，最多约 2 秒）；外部启动的服务器不受影响（鲸仔不托管、不代管）；隐藏到托盘不算退出；鲸仔自更新重启（退出后立即重装再启动）不停服，避免更新期间服务无谓中断
- 「鲸仔设置」→「DeepSeek Harness 服务设置」新增 **开启自动检查更新**（默认开）：关闭后将不再**自动**检查 DeepSeek Harness 更新（后台每小时检查按目标跳过，连版本查询请求都不发）；手动「检查更新」按钮不受影响；鲸仔自身的自动检查不受此项控制
- 「鲸仔设置」设置 tab 新增 **通用设置** 分区（位于 DeepSeek Harness 服务设置之后、其他设置之前）：原「其他设置」里的「开机自启本程序」「显示桌宠」迁入，额外新增 **开启自动检查更新**（默认开）——关闭后将不再**自动**检查鲸仔更新，DeepSeek Harness 的自动检查不受此项控制；手动「检查更新」不受影响
- 管理后台「设置」弹窗同步新增三项开关（服务跟随鲸仔程序停止 / DSH 自动检查更新 / 鲸仔自动检查更新），与内嵌设置分区保持一致——避免该弹窗整体保存时把新配置项重置为默认值
- 前端静默版本查询纳入自动检查开关：关闭 DSH 自动检查后，启动时的初始检查与每 5 分钟轮询一并停用（设置分区手动「检查更新」仍可用）
- 退出逻辑收敛为统一入口（`commands::quit_app`）：托盘「退出」与桌宠右键「退出」共用，置位退出标记后按上述设置决定是否停服
...
- 修复升级后 cordis.patch.yml 出现重复 insert 导致 DSH 服务起不来：同一插件（如同时手动安装 + 鲸仔托管的 WebUI+）被 insert 两次时，DSH 直接报重复插件启动失败——同步时清理标记块外与托管插件 **id 相同** 的残留条目（只按 id 判定，用户手写条目的 name 写法/引号差异不影响去重），标记块内托管版本为权威；标记块外用户安装的其他第三方插件不受影响
- 内置 WebUI+ 插件的 package.json 版本同步到 0.1.2（与 npm 发布一致，此前 0.4.7 内置仍为 0.1.0，会把用户安装的 0.1.2 降级为 0.1.0）
- 备份目录名从秒级改为毫秒级时间戳：同一秒内连续安装不再产生同名备份目录（同名会合并目录并干扰「保留 5 份」剪枝）
- 清理旧备份失败不再误报整个备份失败：备份内容已成功时 prune 报错降级为忽略（best-effort，下次安装再清理）

### 新增
- 安装器支持简体中文：NSIS 安装包内嵌 SimpChinese + English 两种语言，中文系统安装/卸载向导自动显示中文（英文系统自动显示英文）；如需在安装前手动选择语言可开启语言选择对话框
- 安装/更新 DeepSeek Harness 前自动备份 DSH 配置与用户插件：备份 `~/.dsh` 下的 settings.yaml、模型凭证、profiles（排除可重建的 node_modules）、agent 预置，以及应用专用 npm 前缀下用户手动安装的第三方插件（排除 DSH 本体）；备份存到鲸仔应用配置目录 `dsh-backup` 下，保留最近 5 份自动清理，安装日志会打印备份路径供恢复
- 安装/更新失败自动回滚：重装 DSH 前旧版本先暂存（原子 rename，不直接删除），npm 安装或 `dsh --version` 校验失败时自动恢复旧版本——网络/镜像源异常时用户原有 DSH 不再丢失，界面提示「已自动回滚到安装前的版本」
- 安装/更新与启动互斥：`install_dsh`/`update_dsh` 全程持安装锁，期间触发「启动服务器」（托盘菜单/外部事件）会收到「正在安装/更新 DeepSeek Harness，请稍候再启动」，杜绝安装中途启动导致的文件竞争与半装；安装/更新命令执行前若服务器仍在运行（前端漏停或外部启动）自动先强制停止，避免 Windows 文件锁导致重装半途失败
